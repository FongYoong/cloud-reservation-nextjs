...
export const getUserProfile = async (authUser, handler) => {
  firebase.database().ref(`users/${authUser.uid}/profile`).once('value').then((snapshot) => {
...
export const addNewService = async (authUser, data, successHandler, errorHandler) => {
  ...
  Promise.all([updateServices, updateProfile]).then(() => successHandler(serviceId)).catch(errorHandler);
};
...
export const updateService = async (authUser, serviceId, data, successHandler, errorHandler) => {
  firebase.database().ref(`services/${serviceId}/public`).set({
      ...data,
      ownerId: authUser.uid,
  }).then(successHandler).catch(errorHandler);
};

export const updateOrderStatus = async (authUser, serviceId, orderId, data, successHandler, errorHandler) => {
  // initial, accepted, rejected, paidByUser, completed
...
export const deleteService = async (authUser, serviceId, successHandler, errorHandler) => {
  ...
  Promise.all([updateServices, updateProfile]).then(successHandler).catch(errorHandler);
};
